...
type HistoryItem = { id: string; prompt: string; intent: MusicIntent; tracks: Track[]; createdAt: number };
...
export default function HomePage() {
  ...
  function openHistoryItem(id: string) { setSelectedHistoryId(id); navigate('history'); }
  function goBackToHistoryList() { setSelectedHistoryId(null); }
    ...
    <main className="min-h-screen bg-background text-foreground">
      <AppShell view={view} onNavigate={navigate} onSearch={discover} transitionDirection={transitionDirection}>
        {view === 'home' && <div className="mx-auto max-w-5xl">
          <section className="mb-10 pt-3 sm:pt-8"><div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 py-1.5 text-[11px] font-semibold text-primary"><Sparkles size={14} /> AI music discovery</div><h1 className="max-w-3xl text-4xl font-black leading-[1.02] tracking-[-0.04em] sm:text-6xl">Tell PulsyVibe what you feel.</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground sm:text-base">Describe a mood, activity, era, artist, genre, language, or feeling. PulsyVibe finds playable tracks and saves the whole discovery in History. ✨</p></section>
          ...
          <section className="rounded-[1.75rem] border border-white/[0.06] bg-[#0e0e10] p-5 sm:p-6"><div className="flex items-end justify-between gap-4"><div><SectionTitle icon={<History size={17} />} title="Recent history" /><p className="mt-2 text-xs text-muted-foreground">Tap a prompt to reopen the exact songs it fetched.</p></div><button onClick={() => navigate('history')} className="text-xs font-semibold text-primary">See all →</button></div>{history.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2">{history.slice(0, 4).map(item => <button key={item.id} onClick={() => openHistoryItem(item.id)} className="group flex items-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 text-left transition hover:-translate-y-0.5 hover:bg-white/[0.045]"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/[0.05] text-primary"><History size={16} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold">{item.prompt}</span><span className="mt-0.5 block text-[11px] text-white/40">{item.tracks.length} songs · {formatHistoryDate(item.createdAt)}</span></span><span className="text-white/25 transition group-hover:translate-x-0.5">→</span></button>)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-white/[0.08] px-5 py-8 text-center"><div className="text-2xl">🪄</div><p className="mt-2 text-sm font-medium">Your discoveries will appear here.</p><p className="mt-1 text-xs text-muted-foreground">Send your first music prompt above.</p></div>}</section>
        </div>}

        {view === 'search' && <div className="mx-auto max-w-5xl"><DiscoveryForm query={query} setQuery={setQuery} isSearching={isSearching} onSubmit={submit} compact />{isSearching && <LoadingResults query={lastQuery || query} />}{error && !isSearching && <StateCard title="Discovery failed" message={error} action="Try again" onAction={() => void discover(lastQuery)} />}{!isSearching && !error && tracks.length === 0 && <StateCard title="Nothing here yet" message="Describe what you want to hear and PulsyVibe will build a playable result set." action="Go home" onAction={() => navigate('home')} />}{!isSearching && !error && tracks.length > 0 && <><div className="my-5 rounded-[1.75rem] border border-white/[0.06] bg-[#0e0e10] p-5"><div className="flex items-start gap-3"><span className="mt-0.5 rounded-xl bg-primary/10 p-2 text-primary"><Sparkles size={17} /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Your prompt</p><h2 className="mt-1 text-lg font-bold">{intent?.query || lastQuery}</h2><div className="mt-3 flex flex-wrap gap-2 text-[11px] text-muted-foreground">{intent?.mode && <span className="rounded-full bg-white/[0.05] px-2.5 py-1">{intent.mode}</span>}{intent?.languages?.map(item => <span key={item} className="rounded-full bg-white/[0.05] px-2.5 py-1">{item}</span>)}{intent?.genres?.map(item => <span key={item} className="rounded-full bg-white/[0.05] px-2.5 py-1">{item}</span>)}</div></div></div></div><div className="mb-4 flex items-end justify-between"><div><h2 className="text-xl font-bold">{verifiedTracks.length} songs found</h2><p className="mt-1 text-xs text-muted-foreground">Tap any song to play it instantly. ▶️{unavailableCount ? ` · ${unavailableCount} unavailable` : ''}</p></div></div><div className="space-y-1">{tracks.map(track => track.status === 'verified' ? <TrackRow key={track.id} track={track} active={current?.id === track.id} onPlay={() => playTrack(track)} onQueue={() => player.enqueue([track])} onFavorite={() => toggleFavorite(track)} favorite={optimisticFavorites.some(item => item.id === track.id)} onSeed={() => { setQuery(track.title); void discover(track.title); }} /> : <div key={track.id} className="flex items-center gap-3 rounded-2xl p-3 opacity-45"><div className="h-14 w-14 rounded-xl border border-dashed border-white/10" /><div><p className="text-sm font-medium">{track.title}</p><p className="text-xs text-muted-foreground">{track.artist} · unavailable</p></div></div>)}</div></>}</div>}

        {view === 'history' && <div className="mx-auto max-w-5xl">{!selectedHistory ? <><section className="mb-7"><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 py-1.5 text-[11px] font-semibold text-primary"><History size={14} /> Discovery history</div><h1 className="text-3xl font-black tracking-[-0.03em] sm:text-5xl">Your prompts, saved. 🗂️</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Every prompt keeps the original set of songs fetched for it. Open a prompt later and play any song from that exact result set.</p></section>{history.length ? <div className="grid gap-3">{history.map(item => <button key={item.id} onClick={() => openHistoryItem(item.id)} className="group flex items-center gap-4 rounded-[1.35rem] border border-white/[0.06] bg-[#0e0e10] p-4 text-left transition hover:-translate-y-0.5 hover:border-white/[0.11] hover:bg-white/[0.04]"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><History size={19} /></span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold sm:text-base">{item.prompt}</span><span className="mt-1 block text-xs text-white/40">{item.tracks.length} songs · {formatHistoryDate(item.createdAt)}</span></span><span className="rounded-full border border-white/[0.07] bg-white/[0.03] px-3 py-1 text-xs text-white/45">Open →</span></button>)}</div> : <div className="rounded-[1.5rem] border border-dashed border-white/[0.08] bg-[#0e0e10] px-5 py-12 text-center"><div className="text-3xl">🗂️</div><h2 className="mt-3 text-lg font-bold">No history yet</h2><p className="mt-1 text-sm text-muted-foreground">Send a prompt from Discover and it will be saved here.</p><button onClick={() => navigate('home')} className="mt-5 rounded-full bg-primary px-4 py-2 text-xs font-bold text-primary-foreground">Start discovering</button></div>}</> : <><button onClick={goBackToHistoryList} className="mb-5 inline-flex items-center gap-2 text-xs font-semibold text-white/55 hover:text-white"><ArrowLeft size={15} /> Back to history</button><section className="mb-6 rounded-[1.75rem] border border-white/[0.06] bg-[#0e0e10] p-5 sm:p-6"><div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary"><History size={18} /></span><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Saved prompt</p><h1 className="mt-1 text-2xl font-black tracking-[-0.02em] sm:text-3xl">{selectedHistory.prompt}</h1><p className="mt-2 text-xs text-white/40">Fetched {selectedHistory.tracks.length} songs · {formatHistoryDate(selectedHistory.createdAt)}</p></div></div></section><SectionTitle icon={<Sparkles size={17} />} title="Songs from this discovery" /><div className="mt-3 space-y-1">{selectedHistory.tracks.map(track => track.status === 'verified' ? <TrackRow key={track.id} track={track} active={current?.id === track.id} onPlay={() => playTrack(track)} onQueue={() => player.enqueue([track])} onFavorite={() => toggleFavorite(track)} favorite={optimisticFavorites.some(item => item.id === track.id)} onSeed={() => { setQuery(track.title); void discover(track.title); }} /> : <div key={track.id} className="rounded-2xl p-3 opacity-45"><p className="text-sm">{track.title}</p><p className="text-xs text-muted-foreground">{track.artist} · unavailable</p></div>)}</div></>}</div>}
